...
// System sample rate (detected from device at stream open)
std::atomic<float> systemSampleRate(0.0f);
...
int outputCallback(
    ...
    const float sampleRate = systemSampleRate.load();
...
int audioCallback(
    ...
    for (unsigned int i = 0; i < nBufferFrames; i++) {
        ...
        if (std::fabs(sample) > threshold && std::fabs(prev) <= threshold) {
            ...
            double noteTime = now - (double)(nBufferFrames - i) / systemSampleRate.load();
...
void AudioListener(double startTime) {
    ...
    // 1. Get the hardware's favorite speed (e.g., 48000 or 96000)
    unsigned int nativeSR = adc.getDeviceInfo(params.deviceId).preferredSampleRate;
    if (nativeSR == 0) nativeSR = 44100;
    // 2. Suggest 256 as a starting point
    unsigned int bufferFrames = 256;

    try {
        // 3. Open the stream.
        // IMPORTANT: RtAudio uses a pointer (&bufferFrames).
        // If the hardware wants 128 or 512, it WILL change this variable for you!
        adc.openStream(
            nullptr,
            &params,
            RTAUDIO_FLOAT32,
        ...
        adc.startStream();
        // 4. Now bufferFrames is the ACTUAL number the hardware is using.
        systemSampleRate.store((float)nativeSR);
    }
...
void AudioOutput() {
    ...
    unsigned int sampleRate = 0;
    // 1. Get the hardware's favorite speed (e.g., 48000 or 96000)
    unsigned int nativeSR = dac.getDeviceInfo(params.deviceId).preferredSampleRate;
    if (nativeSR == 0) nativeSR = 44100;
    // 2. Suggest 512 as a starting point
    unsigned int bufferFrames = 512;

    try {
        // 3. Open the stream.
        // IMPORTANT: RtAudio uses a pointer (&bufferFrames).
        // If the hardware wants 256 or 1024, it WILL change this variable for you!
        dac.openStream(
            &params,
            nullptr,
            RTAUDIO_FLOAT32,
